...
def jog_pad_source() -> str:
    package_root = Path(__file__).resolve().parent.parent.joinpath("src", "jog_pad")
    module_names = (
        "jog_pad.py",
        "config.py",
        "client.py",
    ...
    return "\n".join((package_root / name).read_text(encoding="utf-8") for name in module_names)
...
def test_clear_cnc_messages_posts_adapter_clear_endpoint(monkeypatch):
    ...
    response = client.clear_cnc_messages()

    assert response == {"status": 0, "message": "ok"}
    assert captured == {
        "url": "http://127.0.0.1:8765/api/cnc/messages/clear",
        "timeout": 1.0,
        "data": None,
        "method": "POST",
    }
...
def test_jog_pad_displays_machine_style_coordinates():
    ...
    assert "POSITION_POLL_INTERVAL_MS = 200" in text
    assert "POSITION_READ_TIMEOUT_SECONDS = 0.75" in text
...
def test_jog_pad_clears_cnc_messages_each_time_it_is_shown():
    text = jog_pad_source()

    assert "def clear_cnc_messages_on_show" in text

...
def test_resolve_home_icon_path_uses_project_home_bitmap(monkeypatch):
    from src.jog_pad import jog_pad
...
def test_coordinate_widgets_use_configured_accent_blue():
    ...
    assert 'ACCENT_BLUE = "#7A4FBF"' in text
...
def test_jog_pad_has_home_status_button():
    ...
    assert "self.position_poller.homed_status_received.connect(self.on_homed_status_received)" in text
...
def test_jog_pad_starts_and_stops_pause_hold_thread():
    ...
    assert "self.jog_pad.pause_hold_thread.stop()" in text
    assert "self.jog_pad.pause_hold_thread.wait(1000)" in text
